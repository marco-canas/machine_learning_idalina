...
Generado automáticamente - 2026-09-04 16:36:41
...
SOLO UTILIZA VARIABLES METEOROLÓGICAS (sin rezagos epidemiológicos)
"""
...
class DengueModelDeployment:
    ...
    def _create_interaction_features(self, X):
        """Crea características de interacción entre variables meteorológicas"""
        ...
        interactions = [
            ...
            ('temp', 'soi', 'temp_soi'),
            ('prec', 'soi', 'prec_soi'),
            ('temp_max', 'prec', 'temp_max_prec'),
            ('hum_rel', 'prec', 'hum_rel_prec'),
            ('vel_vi', 'prec', 'vel_vi_prec'),
        ]
    ...
    def _create_polynomial_features(self, X):
        """Crea características polinomiales para variables clave"""
        ...
        for var in important_vars:
            if var in X.columns:
                ...
                poly_features[var + '_cubed'] = X[var] ** 3

        return poly_features

    def _create_lag_aggregates(self, X):
        """Crea agregados estadísticos de los rezagos meteorológicos"""
        lag_aggs = pd.DataFrame(index=X.index)

        meteo_vars = ['temp', 'temp_max', 'temp_min', 'hum_esp', 'hum_rel', 
                      'prec', 'dias_lluvia', 'vel_vi', 'vel_vi_max', 'vel_vi_min', 'soi', 'sst']

        for base_var in meteo_vars:
    ...
    def _create_cross_lag_features(self, X):
        """Crea características de interacción entre variables y sus rezagos"""
        cross_lag = pd.DataFrame(index=X.index)

        for var in ['temp', 'prec', 'hum_rel', 'soi']:
            if var in X.columns and var + '_lag_4' in X.columns:
                cross_lag[var + '_lag4_ratio'] = X[var] / (X[var + '_lag_4'] + 1)

            if var in X.columns and var + '_lag_8' in X.columns:
                cross_lag[var + '_lag8_ratio'] = X[var] / (X[var + '_lag_8'] + 1)
    ...
    def _augment_features(self, X):
        ...
        cross_lag_features = self._create_cross_lag_features(X_numeric)
        if not cross_lag_features.empty:
            features_list.append(cross_lag_features)
...
if __name__ == "__main__":
    ...
    print("SOLO CON VARIABLES METEOROLÓGICAS")
    print("="*60)

    # Ruta del modelo
    model_dir = r"C:\Users\marco\Documentos\investigacion\machine_learning_idalina\3_xgboost\2_datos\1_raw\1_80_20\modelo_guardado_meteo"
    ...
    test_path = r"C:\Users\marco\Documentos\investigacion\machine_learning_idalina\3_xgboost\2_datos\1_raw\1_80_20\3_meteo_epi_2021-2026_1_rezagos_meteo_test_80_20.xlsx"
